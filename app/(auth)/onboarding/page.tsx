        ...
        {step.type === "radio" && step.options && (
          <div className="space-y-3">
            {step.options.map((opt) => {
              const isSelected =
                step.name === "skin-type"
                  ? answers.skin_type === opt.value
                  : step.name === "acne-severity"
                    ? answers.acne_severity === opt.value
                    : answers.goal === opt.value;
              return (
                <label
                  key={opt.value}
                  className={`btn-press flex items-center gap-4 p-4 bg-white border-2 rounded-2xl cursor-pointer hover:border-primary/30 transition-all ${
                    isSelected
                ...
                >
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${isSelected ? "border-primary bg-primary" : "border-slate-300"}`}>
                    <svg className={`w-3 h-3 text-white ${isSelected ? "opacity-100" : "opacity-0"}`} fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                  </div>
                </label>
              );
            })}
